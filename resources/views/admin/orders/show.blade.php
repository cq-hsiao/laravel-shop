    ...
    <div class="box-body">
        <table class="table table-bordered">
            ...
            <tr>
                ...
                <td>支付时间：</td>
                <td>@if($order->paid_at){{ $order->paid_at->format('Y-m-d H:i:s') }}@else -- @endif</td>
            </tr>
            <tr>
                <td>支付方式：</td>
                ...
                $order->items[0]->product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_SUCCESS))
                    ...
                    <td colspan="4">
                        <form action="{{ route('admin.orders.ship', [$order->id]) }}" method="post" class="form-inline">
                            <!-- 别忘了 csrf token 字段 -->
                            {{ csrf_field() }}
                            <div class="form-group {{ $errors->has('express_company') ? 'has-error' : '' }}" style="height: 60px;">
                                <label for="express_company" class="control-label">物流公司</label>
                                <input type="text" id="express_company" name="express_company" value="{{old('express_company',$order->express_company)}}" class="form-control" placeholder="输入物流公司">
                            ...
                            <div class="form-group {{ $errors->has('express_no') ? 'has-error' : '' }}" style="height: 60px;">
                            ...
                            <button type="submit" class="btn btn-success" id="ship-btn" style="position: absolute;margin-left:10px; ">发货</button>
                        </form>
                    </td>
                </tr>
                @endif
    ...
    $(document).ready(function() {
        ...
        $('#btn-refund-agree').click(function () {
            swal({
                ...
                swal('操作失败', error.responseJSON.msg, 'error');
            });
        });
    });
</script>