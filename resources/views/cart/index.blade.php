            ...
            <div class="card">
                ...
                <div class="card-body">
                    <table class="table table-striped">
                        <thead>
                        <tr>
                            <th><input type="checkbox" id="select-all" checked="checked"></th>
                            <th>商品信息</th>
                            <th>单价</th>
                            <th>数量</th>
                            <th>操作</th>
                        ...
                        <form class="form-horizontal" role="form" id="order-form">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9 col-md-7">
                                    ...
                                    <a class="btn btn-sm btn-success" style="line-height: 2.2;" href="{{route('user_addresses.create')}}">新增地址</a>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('input[name=select][type=checkbox]').change(function() {
                var len = $('input[name=select][type=checkbox]:not([disabled])').length;
                var choice_len = $('input[name=select][type=checkbox]:not([disabled]):checked').length;
            ...
            $('.btn-create-order').click(function () {
                ...
                $('input[name=select][type=checkbox]:not([disabled]):checked').each(function () {
                    // 获取当前行中数量输入框
                    var $input = $(this).parents('tr').find('input[name=amount]');
                    // 如果用户将数量设为 0 或者不是一个数字，则也跳过
                    if ($input.val() == 0 || isNaN($input.val())) {
                        return;
                    }
                    ...
                    var $name = $(this).parents('tr').find('span[class=product_title] a').html() + '[' + $(this).parents('tr').find('span[class=sku_title]').html() + ']';

                    // 把 SKU id 和数量存入请求参数数组中
                    req.items.push({
                        sku_id : $(this).parents('tr').data('id'),
                        amount : $input.val(),
                        name : $name
                    });
                    console.log(req);
            ...
            $('#btn-check-coupon').click(function () {
                ...
                axios.get('coupon_codes/' + encodeURIComponent(code))
                    .then(function (res) {
                        $('#coupon_desc').text(res.data.description); // 输出优惠信息
