<?php

namespace App\Admin\Controllers;

use App\Models\CouponCode;
use Encore\Admin\Facades\Admin;
use Encore\Admin\Controllers\AdminController;
use Encore\Admin\Form;
use Encore\Admin\Grid;
use Encore\Admin\Layout\Content;


class CouponCodesController extends AdminController
{

    public function index(Content $content)
    {
        return $content
            ->header('优惠券列表')
            ->body($this->grid());
    }

    /**
     * Make a grid builder.
     *
     * @return Grid
     */
    protected function grid()
    {
        $grid = new Grid(new CouponCode);

        // 默认按创建时间倒序排序
        $grid->model()->orderBy('created_at', 'desc');
        $grid->id('ID')->sortable();
        $grid->name('名称');
        $grid->code('优惠码');
//        $grid->type('类型')->display(function($value) {
//            return CouponCode::$typeMap[$value];
//        });
//        // 根据不同的折扣类型用对应的方式来展示
//        $grid->value('折扣')->display(function($value) {
//            return $this->type === CouponCode::TYPE_FIXED ? '￥'.$value : $value.'%';
//        });
        $grid->description('描述');
//        $grid->min_amount('最低金额');
//        $grid->total('总量');
//        $grid->used('已用');
        $grid->column('usage', '用量')->display(function ($value) {
            return "{$this->used} / {$this->total}";
        });
        $grid->enabled('是否启用')->sortable()->display(function($value) {
            return $value ? '是' : '否';
        });
        $grid->created_at('创建时间');

        $grid->actions(function ($actions) {
//            $actions->disableView();
        });

        return $grid;
    }

    protected function form()
    {
        $form = new Form(new CouponCode());

        $form->tools(function (Form\Tools $tools) {

            // 去掉`查看`按钮
//            $tools->disableView();
        });

        $form->display('id','ID');
        $form->text('name', '名称')->rules('required');
//        $form->text('code', '优惠码')->rules('nullable|unique:coupon_codes');
        $form->text('code', '优惠码')->rules(function ($form){
            // 如果 $form->model()->id 不为空，代表是编辑操作
            if($id = $form->model()->id){
                return 'nullable|unique:coupon_codes,code,'.$id.',id';
            } else {
                return 'nullable|unique:coupon_codes';
            }
        });

        $form->radio('type', '类型')->options(CouponCode::$typeMap)->rules('required')->default(CouponCode::TYPE_FIXED);
        $form->text('value', '折扣')->rules(function ($form) {
            if (request()->input('type') === CouponCode::TYPE_PERCENT) {
                // 如果选择了百分比折扣类型，那么折扣范围只能是 1 ~ 99
                return 'required|numeric|between:1,99';
            } else {
                // 否则只要大等于 0.01 即可
                return 'required|numeric|min:0.01';
            }
        });
        $form->text('total', '总量')->rules('required|numeric|min:0');
        $form->text('min_amount', '最低金额')->rules('required|numeric|min:0');
        $form->datetime('not_before', '开始时间');
        $form->datetime('not_after', '结束时间')->rules(function ($form){
            if(request()->input('not_before') && request()->input('not_after')){
                return 'date|after:'.request()->input('not_before');
            } else {
                return false;
            }
        });
        $form->radio('enabled', '启用')->options(['1' => '是', '0' => '否']);

        $form->saving(function (Form $form) {
            if (!$form->code) {
                $form->code = CouponCode::findAvailableCode();
            }
        });

//        $form->disableViewCheck();

        return $form;
    }


    public function show($id, Content $content)
    {
        return $content
            ->header('查看优惠券')
            // body 方法可以接受 Laravel 的视图作为参数
            ->body(Admin::show(CouponCode::query()->findOrFail($id),function($show){
                $show->id('ID');
                $show->name('优惠券名称');
                $show->code('优惠码');
                $show->type('类型')->as(function($value) {
                    return CouponCode::$typeMap[$value];
                });

                // 根据不同的折扣类型用对应的方式来展示
                $show->value('折扣')->as(function($value){
                    return $this->type === CouponCode::TYPE_FIXED ? '￥'.$value : $value.'%';
                });
                $show->description('描述');

                $show->enabled('是否启用')->as(function($value) {
                    return $value ? '是' : '否';
                });
                $show->usage('用量')->as(function ($value) {
                    return "{$this->used} / {$this->total}";
                });
            }));
    }




}
